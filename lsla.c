...
int main(int argc, char * argv[])                                               // argc = 2, compiled_program directory_name
{
    ...
    if( stat(argv[1], &object_statistic) != 0 )                                 // trying to find bugs with stat function ( segmentation fauled or file doesnt exist)
    {
        perror("Error! Reason == >  ");
        return 1;
    } 
    ...
    if((object_statistic.st_mode & S_IFMT) == S_IFREG)                          // checking argument with 1 index for being a regular file
    {
        struct passwd *user_id_statistic;                                       // passw structure. contains password and user's real name
        struct group  *group_id_statistic;                                      // grouo structrue. contains real name of group, i need it
        ...
        string_pointer[helpful_size - 1] = '\0'; 
        putchar('-');
        ...
        user_id_statistic = getpwuid(object_statistic.st_uid);                   // returning pointer to pwd structrue 
    ...
    else if( (object_statistic.st_mode & S_IFMT) == S_IFDIR)
    {
